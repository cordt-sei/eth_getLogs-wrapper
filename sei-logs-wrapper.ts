 ...
 */
function toHexWithPrefix(value: string | number): string {
    ...
    try {
        ...
        if (logs.length === 10000) {
            ...
            const [firstHalfLogs, secondHalfLogs] = await Promise.all([
                fetchLogsRecursive(
                    { ...normalizedFilter, toBlock: toHexWithPrefix(midBlock) },
                    rpcUrl,
                    method
                ),
                fetchLogsRecursive(
                    { ...normalizedFilter, fromBlock: toHexWithPrefix(midBlock + 1) },
                    rpcUrl,
                    method
                ),
            ]);
 ...
 */
export async function fetchAndStandardizeLogs(
    filter: LogFilter,
    rpcUrl: string,
    ...
    try {
        ...
        const blockLogsMap = new Map<string, number>();
